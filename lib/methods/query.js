...
exports.query = function (Model, options, callback) {
	...
	if (sel && Object.keys(sel).length > 0) {
		...
		keys += ', ' + this.escapeKeys(_.keys(_.omit(_.omit(this.getTableSchema(Model), primaryKeyColumn), _.keys(unsel).map(toUpperCase)))).join(', ');
	} else {
		keys = '*';
	}
	...
	pagingQuery += ' ' + (whereQuery ? 'AND' : 'WHERE') + ' ROWNUM >= ' + (+options.skip || 0) + ' AND ROWNUM <= ' + (+options.limit);

	var query = 'SELECT ' + keys + ' FROM ' + table + whereQuery + pagingQuery + orderQuery;
	this._query(query, values, {useResultSet: true}, callback, function (rows, results) {
		if (rows && rows.length) {
			callback(null, new Arrow.Collection(Model, rows.map(function (row) {
