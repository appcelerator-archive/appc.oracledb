...
exports.fetchSchema = function (next) {
  ...
  function getConstraints() {
    var query = 'SELECT ' + 'cols.TABLE_NAME, ' + 'cols.COLUMN_NAME, ' + 'cons.STATUS, ' + 'cons.CONSTRAINT_TYPE ' + 'FROM user_constraints cons, user_cons_columns cols ' + 'WHERE cons.CONSTRAINT_NAME = cols.CONSTRAINT_NAME';
    self._query(query, { useResultSet: true }, next, function (results) {
      constraints = results;
      getColumns();
    });
  }
   ...
   */
  function getColumns() {
    var query = 'SELECT ' + 'TABLE_NAME, ' + 'COLUMN_NAME, ' + 'DATA_TYPE, ' + 'DATA_LENGTH, ' + 'DATA_PRECISION, ' + 'NULLABLE ' + 'FROM user_tab_columns';
    self._query(query, { useResultSet: true }, next, function (results) {
      columns = results;
      calculateSchema();
    });
