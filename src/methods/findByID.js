...
exports.findByID = function (Model, id, callback) {
  ...
  var query = 'SELECT ' + (primaryKeyColumn && keys.indexOf(primaryKeyColumn.toUpperCase()) === -1 ? primaryKeyColumn + ', ' : '') + this.escapeKeys(keys).join(', ') + ' FROM ' + table + ' WHERE ROWNUM <= 1 AND "' + primaryKeyColumn + '" = :id';
  if (!primaryKeyColumn) {
