...
exports.findAll = function (Model, callback) {
  ...
  var query = 'SELECT ' + (primaryKeyColumn && keys.indexOf(primaryKeyColumn.toUpperCase()) === -1 ? primaryKeyColumn + ', ' : '') + this.escapeKeys(keys).join(', ') + ' FROM ' + table + (primaryKeyColumn ? ' ORDER BY ' + primaryKeyColumn : '');
